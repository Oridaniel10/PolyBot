...
| 13c | **Momentum confirmation delay** | First momentum/double signal → record pending, **SKIP** `momentum_pending_confirmation`; after `momentum_confirmation_delay_sec` (default 60s), if signal still qualifies → **BUY** at live price; else pending ages out (`momentum_confirmation_max_age_sec`). Set delay `0` for immediate buy. | `MOMENTUM_CONFIRMATION_DELAY_SEC`, `MOMENTUM_CONFIRMATION_MAX_AGE_SEC` | `decision_core.py::check_momentum_confirmation` |
